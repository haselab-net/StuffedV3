...
#include "include/dukf_utils.h"
#include "duktape_utils.h"
#include "logging.h"
#include "modules.h"
#include "duk_alloc_hybrid.h"

#include "include/module_iot.h"
#include "include/module_callbacks.h"
...
static xSemaphoreHandle heap_mutex = NULL;	// mutex for heap
...
void dukInitTask(){
	lock_heap();
	createDuktapeHeap();
	TaskHandle_t task = jsThreads[0].task;
	jsThreads[0].task = NULL;
	unlock_heap();
	vTaskDelete(task);
}
...
void duktape_start() {
	...
	initModuleDevice();
	#if 1
	xTaskCreate(dukInitTask, "jsInit", 1024*10, NULL, tskIDLE_PRIORITY+1, &jsThreads[0].task);
	#else
	createDuktapeHeap();
	#endif
	while(jsThreads[0].task){
		vTaskDelay(1);
	}
	vTaskDelay(1);
	lock_heap();
