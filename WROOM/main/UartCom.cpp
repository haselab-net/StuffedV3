...
void Uart::Init(uart_config_t conf, int rxPin, int txPin){
	...
	uart_driver_install(port, 256, 256, 4, NULL, 0);	//	とりあえず、Queueを使わずに動かしてみる。後で全面的に直したい。
}
void Uart::Loop() {
	//	receive first
	size_t rxLen;
...
#if 0
		char buf[200];
		sprintf(buf, "B%dR%d %d ", retCur.board, retCur.cur, boards[retCur.board]->RetStart()[retCur.cur]);
		Serial.println(buf);
#endif
		retCur.cur++;
